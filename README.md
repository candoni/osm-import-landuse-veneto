...
Questo progetto è una rielaborazione del lavoro documentato su
[wiki.openstreetmap.org — Import dalla CTRN Veneto a OSM](https://wiki.openstreetmap.org/wiki/Veneto/Guide_e_documentazione/Import:_dalla_CTRN_Veneto_a_OSM).

Partendo dai dati aperti di **Copertura del Suolo 2023** (layer 14275) pubblicati da
Regione Veneto sul portale IDT2, il progetto produce file `.osm` pronti per l'importazione
in OpenStreetMap — uno per ogni comune del Veneto — e un'app web interattiva per
visualizzare e confrontare i dati regionali con quelli già presenti in OSM.
...
Apri l'app online (pubblicata su GitHub Pages) per visualizzare, comune per comune, i
poligoni di copertura del suolo con i tag OSM convertiti. Puoi passare tra tre viste:
...
- **Diff** — un confronto _indicativo_ che mostra le corrispondenze, le discordanze e le
  aree mancanti in OSM 

---

### 2. Importa i dati di un comune in JOSM

I file `.osm` pronti all'uso si trovano nella cartella `dist/copsuolo_2023_14275/`
...
1. Scegli il comune che ti interessa, ad esempio
   `dist/copsuolo_2023_14275/VI/vicenza_24116.osm`
2. Aprilo in JOSM 
...
La conversione dei codici regionali in tag OSM è interamente definita nel file
[`src/copsuolo/translation/map_copsuolo.yaml`](src/copsuolo/translation/map_copsuolo.yaml).
...
- `clc_lv_all` è il codice della classe regionale, come descritto in
  `docs/Specifiche_di_contenuto.pdf`. `description` viene dallo stesso file.
- `tags` contiene i tag OSM corrispondenti che verranno applicati

Se noti che un tipo di terreno è mappato in modo errato o incompleto, apri una issue o
proponi una modifica a `src/copsuolo/translation/map_copsuolo.yaml`. 
...
Se vuoi rigenerare i file `.osm` o ricostruire le mappe da zero, clona il repository e
segui le istruzioni nella sezione [Sviluppo](#sviluppo) qui sotto.
...
I dati vengono salvati in `data/copsuolo_2023_14275/{PROVINCIA}/`, uno shapefile per
comune. Gli zip temporanei finiscono in `data/tmp/`. Lo script aspetta 1 secondo tra un
comune e il successivo (modificabile con `--sleep-seconds`).
...
Il viewer frontend usa lo stesso link fisso nel footer per "Dati regionali":
...
Esegue `ogr2osm` su ogni shapefile trovato sotto `data/copsuolo_2023_14275/` usando
`src/copsuolo/translation/landuse.py` come file di traduzione (che legge i mapping da `src/copsuolo/translation/map_copsuolo.yaml`).
I file `.osm` vengono scritti in `dist/copsuolo_2023_14275/`.
...
Il layer di confronto richiede un estratto `.pbf` locale (default:
`data/italy-nord-est.osm.pbf`) disponibile ad esempio su [geofabrik](https://download.geofabrik.de/europe/italy/nord-est.html). 
Ogni comando salta automaticamente le province già aggiornate.
...
Il repository include un workflow GitHub Actions che pubblica automaticamente il frontend
